...
/**
 * Class describing the functionality of an electrical Item on the board with a shape on 1 or several layers.
 */
public class Pin extends DrillItem implements Serializable {
  ...
  /**
   * Calculates the allowed trace exit directions of the shape of this padstack on layer p_layer together with the minimal trace line lengths into their directions. Currently implemented only for box
   * shapes, where traces are allowed to exit the pad only on the small sides.
   */
  ...
  /**
   * Returns true, if vias throw the pads of this pins are allowed, false, otherwise. Currently, drills are allowed to SMD-pins.
   */
  ...
  /**
   * Returns the neckdown half width for traces on p_layer. The neckdown width is used, when the pin width is smaller than the trace width to enter or leave the pin with a trace.
   */
  ...
  /**
   * Calculates the nearest exit restriction direction for changing p_trace_polyline. p_trace_polyline is assumed to start at the pin center. Returns null, if there is no matching exit restrictions.
   */
