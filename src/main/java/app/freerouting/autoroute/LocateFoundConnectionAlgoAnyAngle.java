...
/**
 * Calculates from the backtrack list the location of the traces and vias, which realize a connection found by the maze search algorithm.
 */
class LocateFoundConnectionAlgoAnyAngle extends LocateFoundConnectionAlgo {
    ...
    for (int i = check_from_door_index; i < new_door_ind; i++) {
      ...
      if (Math.abs(curr_dist) < trace_halfwidth_middle) {
        ...
        if (curr_corrected_result != null) {
          if (corrected_result == null || curr_corrected_result.side_of(this.current_from_point, corrected_result) == Side.ON_THE_RIGHT) {
      ...
      if (Math.abs(curr_dist) < trace_halfwidth_middle) {
        ...
        if (curr_corrected_result != null) {
          if (corrected_result == null || curr_corrected_result.side_of(this.current_from_point, corrected_result) == Side.ON_THE_LEFT) {
            corrected_door_ind = i;
            corrected_result = curr_corrected_result;
          }
        }
  ...
  /**
   * Calculates as first line the left side tangent from p_from_corner to the circle with center p_to_corner and radius p_dist. As second line the right side tangent from p_to_corner to the circle
   * with center p_next_corner and radius 2 * p_dist is constructed. The second line is than translated by the distance p_dist to the left. Returned is the intersection of the first and the second
   * line.
   */
  ...
  /**
   * Calculates as first line the right side tangent from p_from_corner to the circle with center p_to_corner and radius p_dist. As second line the left side tangent from p_to_corner to the circle
   * with center p_next_corner and radius 2 * p_dist is constructed. The second line is than translated by the distance p_dist to the right. Returned is the intersection of the first and the second
   * line.
   */
